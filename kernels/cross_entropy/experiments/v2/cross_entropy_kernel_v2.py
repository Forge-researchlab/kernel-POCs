"""
Forge Cross Entropy v2.

This v2 experiment ports the full Liger cross entropy feature surface into the
Forge POC tree instead of benchmarking only the unweighted core path. It keeps
the Liger execution contract because that is the point of the comparison:
forward computes the loss and, when gradients are required, writes
...
def _element_mul_kernel(
    ...
    values = tl.load(X_ptr + row * X_stride + offsets, mask=mask)
    tl.store(X_ptr + row * X_stride + offsets, values * grad_output, mask=mask)


@triton.jit
def forge_cross_entropy_kernel(
    X_ptr,
    X_stride,
    Y_ptr,
    Y_stride,
...
# Keep the same CUDA default Liger uses. The cap is below Triton's hard tensor
# limit and avoids the register pressure seen with larger row blocks.
...
class ForgeCrossEntropyFunction(torch.autograd.Function):
    """
    This class implements a custom autograd function for the Forge Cross Entropy loss.
    ...
    def forward(
        ...
        """
        The forward pass of the Forge Cross Entropy loss.
    ...
    def backward(ctx, grad_output, grad_output2, grad_output3, grad_output4):
        """
        The backward pass of the Forge Cross Entropy loss.
...
def forge_cross_entropy(
    ...
    """Functional Forge cross entropy API for the v2 experiment."""

    loss, z_loss, token_accuracy, predicted_tokens = ForgeCrossEntropyFunction.apply(
        _input,
        target,
        weight,
        ignore_index,
...
class ForgeCrossEntropyLoss(torch.nn.Module):
    def __init__(
        self,
        weight: Optional[torch.FloatTensor] = None,
        ignore_index: int = -100,
