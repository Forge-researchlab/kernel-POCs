...
# DTensor moved from torch.distributed._tensor (torch < 2.5) to
# torch.distributed.tensor (torch >= 2.5). Provide a single _DTensor name that
# resolves to the available class, or to a never-matches sentinel if neither
# path exists (e.g. distributed not built).
try:
    from torch.distributed.tensor import DTensor as _DTensor  # torch >= 2.5
except (ImportError, AttributeError):
    try:
        from torch.distributed._tensor import DTensor as _DTensor  # torch < 2.5
    except (ImportError, AttributeError):
        class _DTensor:  # pragma: no cover — sentinel for no-distributed builds
            pass
