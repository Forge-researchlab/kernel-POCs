"""LoRA fine-tune Gemma 2-2B on the pirate-speak dataset under FSDP2 + forge.patch.

Demo for the Forge team: real google/gemma-2-2b weights, LoRA r=16, our §7-fixed
...
Plotting + inference are separate scripts (plot_artifacts.py, run_inference.py).

Run:
    HF_HOME=/workspace/.hf-cache \
      torchrun --nproc-per-node=2 --standalone \
      forge/demos/train_lora_gemma_forge.py
"""
...
TRAIN_STEPS = 200
LR = 2e-4
BATCH = 2                # per-microbatch sequences
MICRO_BATCHES = 4        # gradient accumulation -> effective batch 8
MAX_SEQ_LEN = 96
...
def encode_example(tokenizer, english: str, pirate: str, max_len: int):
    """Return (input_ids, labels) tensors with labels masked to -100 over the
    prompt so loss only flows through the completion tokens."""
...
def build_peft_model(device, dtype):
    """Load real Gemma 2-2B + wrap with PEFT LoRA on q/k/v/gate/up/down."""
    ...
    # PEFT 0.19.1 inits lora_A/lora_B in fp32 even when the base layer is bf16.
    # FSDP2 requires uniform param dtype, so cast everything to dtype here.
...
def main():
    ...
    if is_rank0:
        ...
        print("=" * 70)
        print(f"Forge LoRA fine-tune: Gemma 2-2B + forge.patch + FSDP2")
        print("=" * 70)
    ...
    # Build model + tokenizer (each rank loads same weights from cache)
    t0 = time.time()
    peft_model, tokenizer = build_peft_model(device, dtype)
    if is_rank0:
        print(f"  loaded {MODEL_ID} in {time.time()-t0:.1f}s")

    # FSDP2 wrap: each Gemma2DecoderLayer + the root.
    ...
    # forge.patch AFTER FSDP2 wrap (locked order per phase3_fsdp2_resume.md §10).
    ...
    # Reset peak before training so VRAM stats are training-only.
    ...
    for step in range(1, TRAIN_STEPS + 1):
        ...
        # Per-rank peak memory at this moment (high-water-mark over training).
        peak_gb = torch.cuda.max_memory_allocated() / 1e9
        # Gather peaks across ranks for the JSONL log.
        peaks = [None] * world
    ...
    # Save LoRA adapter from rank 0. PEFT's save_pretrained() walks the model
    # and gathers DTensors automatically (PEFT >= 0.10).
    if is_rank0:
        ADAPTER_DIR.mkdir(parents=True, exist_ok=True)
        # Gather state dict for adapter weights only.
        gathered = {}
        ...
        # Non-zero ranks still need to call full_tensor() collectively.
        for name, p in peft_model.state_dict().items():
            if "lora_" not in name:
                continue
            if hasattr(p, "full_tensor"):
    ...
    if is_rank0:
        ...
        # Also save the LoraConfig so inference can rebuild it.
