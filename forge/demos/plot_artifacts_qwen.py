...
def plot_step_time(rows, meta, out_path):
    steps = np.array([r["step"] for r in rows])
    times = np.array([r["step_time_s"] for r in rows]) * 1000
    cumulative = np.cumsum(times) / 1000
...
def plot_summary_dashboard(rows, meta, out_path):
    ...
    ax.set_title("Peak VRAM per rank (high-water mark)", fontsize=11)
    ax.grid(True, alpha=0.25)
...
def main():
    rows = load_log()
    meta = load_meta()
    print(f"Loaded {len(rows)} step rows from {LOG_PATH}")
    ...
    print(f"Wrote 4 PNGs to {ARTIFACTS_DIR}")
