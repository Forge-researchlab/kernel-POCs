"""FSDP2 + ALL kernels + LoRA + Gemma 2 multi-GPU smoke test (H2).

Companion to verify_fsdp2_lora_gemma.py — instead of patching only
{lora_qkv, lora_mlp}, this version applies `forge.patch(model)` with no
whitelist so every real kernel for Gemma 2 is exercised under FSDP2:
rmsnorm, geglu (MLP fallthrough), lora_qkv, lora_mlp, embedding, plus the
module-level apply_rotary_pos_emb replacement.

Surfaces closure-over-weight bugs in non-LoRA kernels (rmsnorm, embedding)
that the LoRA-only test couldn't reach.

Run:
    torchrun --nproc-per-node=2 --standalone forge/tests/verify_fsdp2_all_kernels_gemma.py
"""
...
def run_fsdp2(rank, world, device, dtype, init_path):
    ...
    # H2: no whitelist — patch every kernel with a real impl.
    forge.patch(peft_model)
    if rank == 0:
...
def _evaluate(logits, gen, gen_ok, losses, grads, ref):
    ...
    for key in ("q_lora_A", "q_lora_B", "gate_lora_A", "gate_lora_B"):
        ...
        if trivial:
            ...
            c = _cos(a, b)
            # Looser cos threshold than the LoRA-only test (0.999 vs 0.9999):
            # all-kernels stacks bf16 reduction-order noise from rmsnorm +
            # embedding + RoPE on top of the LoRA gradient, pushing cos by
            # ~1e-4. See resume doc §11 Footgun E. rel threshold unchanged.
            ok = (rel < 5e-2 and c > 0.999)
...
def main():
    ...
    if rank == 0:
        print("=" * 70)
        print("FSDP2 + ALL kernels + LoRA + Gemma 2 (H2)")
        print("=" * 70)
