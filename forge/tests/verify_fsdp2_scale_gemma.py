"""FSDP2 + LoRA + Gemma 2 at REAL dimensions (H3).

Companion to verify_fsdp2_lora_gemma.py — scales the tiny smoke (hidden=128
num_layers=4) up to roughly Gemma 2-2B dimensions (hidden=2304 num_layers=18,
GQA 8/4 heads, head_dim=256). At this scale FSDP2 memory sharding becomes
visible — C4 per-rank peak should be substantially below the single-GPU
reference peak. bf16 reduction-order noise is also more pronounced, so
thresholds are looser than the tiny-model test.

Random init (no real Gemma 2 weights download); seed is shared across ranks.

Run:
    torchrun --nproc-per-node=2 --standalone forge/tests/verify_fsdp2_scale_gemma.py
"""
...
SEED = 0
# Roughly Gemma 2-2B dimensions, scaled for fast iteration:
HIDDEN = 2304
INTERMEDIATE = 9216
NUM_LAYERS = 18              # full Gemma 2-2B is 26; 18 keeps step time tractable
VOCAB = 32000                # full Gemma 2 is 256K; 32K is closer to common tokenizers
BATCH = 2
SEQ = 128                    # larger seq so activation memory matters
LR = 1e-4                    # smaller LR for stability at scale
TRAIN_STEPS = 5
MICRO_BATCHES = 4
GEN_TOKENS = 16
...
def _build_peft_model(device, dtype, seed=SEED):
    ...
    cfg = Gemma2Config(
        ...
        num_attention_heads=8,            # Gemma 2-2B has 8 heads
        num_key_value_heads=4,            # GQA 8/4
        head_dim=256,                     # Gemma 2 head_dim is 256
...
def _evaluate(logits, gen, gen_ok, losses, grads, ref):
    ...
    # At scale (hidden=2304, 18 layers) bf16 reduction-order noise on logits
    # is larger than the tiny-model floor — Footgun E observed ~5e-2 worst case.
    results["C2_forward"] = (diff < 5e-1 and cos > 0.999)
    ...
    for key in ("q_lora_A", "q_lora_B", "gate_lora_A", "gate_lora_B"):
        ...
        if trivial:
            ...
            c = _cos(a, b)
            # Looser thresholds at scale — bf16 noise compounds across 18 layers.
            ok = (rel < 1e-1 and c > 0.999)
    ...
    if not gen_ok or not ref["gen_ok"] or gen is None or ref["ref_gen"] is None:
        ...
        # At scale, bf16 tie-breaking on argmax can flip a few tokens. We
        # compare 128 input + 16 new = 144 tokens; allow up to 4 flips on
        # the 16 generated tokens (input prefix should always match).
        results["C5_generation"] = (n_match >= gen.shape[1] - 4)
    ...
    # Looser convergence thresholds at scale (per-step rel may be 5% with bf16).
    results["C6_training"] = (
        converging_ref and converging_new
        and max(rel_diffs) < 5e-2
        and final_rel < 1e-1
    )
